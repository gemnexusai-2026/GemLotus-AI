...
function createFinding(
  ...
  return {
    id: `factory-finding-${Date.now()}-${Math.random()
      .toString(36)
      .slice(2, 8)}`,
...
export default function FactoryFindings({
  factory,
  onChange,
}: FactoryFindingsProps) {
  const findings =
    factory.findings ?? [];
  ...
  function addFinding() {
    onChange([
      ...findings,
      createFinding(factory.id),
    ]);
  }

  function updateFinding(
    index: number,
    patch: Partial<FactoryFinding>,
  ) {
    const next = [...findings];
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <h2 className="mt-2 text-lg font-semibold text-white">
              Factory Findings & Corrective Actions
            </h2>

            <p className="mt-2 max-w-3xl text-[10px] leading-5 text-white/30">
              Record factory infrastructure gaps,
              classify severity and track corrective
              action closure.
            </p>
          </div>
        ...
        {findings.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/[0.08] bg-black/[0.12] p-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-[10px] leading-5 text-white/25">
              No factory infrastructure findings
              have been recorded.
            </p>
          </div>
        ) : (
                ...
                >
                          ...
                          updateFinding(
                            index,
                            {
                              severity:
                                event
                                  .target
                                  .value as FactoryFindingSeverity,
                            },
                          )
                        }
                  ...
                  <div className="mt-4">
                    <TextArea
                      ...
                      placeholder="Describe the observed infrastructure gap..."
                    />
                  </div>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    <TextArea
                      ...
                      placeholder="Define corrective action required..."
                    />
                          ...
                          updateFinding(
                            index,
                            {
                              correctiveActionStatus:
                                event
                                  .target
                                  .value as FactoryFinding["correctiveActionStatus"],
                            },
                          )
                        }
                        className="w-full rounded-xl border border-white/[0.08] bg-[#071426] px-4 py-3 text-sm text-white outline-none focus:border-cyan-300/30"
