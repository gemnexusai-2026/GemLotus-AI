...
export default function ProductFindings({
  product,
  onChange,
}: ProductFindingsProps) {
  const findings = product.findings ?? [];

  function addFinding() {
    onChange([
      ...findings,
      createFinding(product.id),
    ]);
  }

  function updateFinding(
    index: number,
    patch: Partial<ProductFinding>,
  ) {
    const next = [...findings];
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-lg font-semibold text-white">
              Product Findings & Corrective Actions
            </h2>

            <p className="mt-2 max-w-3xl text-[10px] leading-5 text-white/30">
              Record product-level gaps, classify
              their severity and track corrective
              action closure.
            </p>
          </div>
        ...
        {findings.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/[0.08] bg-black/[0.12] p-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-[10px] leading-5 text-white/25">
              No product-level corrective findings
              have been recorded for this assessment.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateFinding(index, {
                          severity:
                            event.target
                              .value as ProductFindingSeverity,
                        })
                      }
                  ...
                  <Field
                    ...
                    placeholder="Evidence ID / document"
                  />
                </div>
                ...
                <div className="mt-4">
                  <TextArea
                    ...
                    placeholder="Describe the observed gap or non-conformity..."
                  />
                </div>
                      ...
                      onChange={(event) =>
                        updateFinding(index, {
                          correctiveActionStatus:
                            event.target
                              .value as ProductFinding["correctiveActionStatus"],
                        })
                      }
                      className="w-full rounded-xl border border-white/[0.08] bg-[#071426] px-4 py-3 text-sm text-white outline-none focus:border-cyan-300/30"
                    >
...
function StatusBadge({
  ...
  status: ProductFinding["correctiveActionStatus"];
}) {
  const classes =
    status === "closed"
